...
export default function ProductCard({ product }: Props) {
  ...
  const priceLabel = lowestPrice !== null
    ? `${(product.variants?.length ?? 0) > 1 ? "From " : ""}${formatPrice(lowestPrice, "USD")}${compareAtPrice ? `, was ${formatPrice(compareAtPrice, "USD")}` : ""}`
    : ""
  ...
  return (
    <article>
      <Link
        ...
        aria-label={`${product.title}${priceLabel ? ` — ${priceLabel}` : ""}`}
      ...
      >
        <div className="relative aspect-square overflow-hidden rounded-xl bg-white border border-border mb-3">
          {firstImage ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-muted-foreground text-sm" aria-hidden="true">
              No image
            </div>
          )}
        </div>
        <h3 className="font-medium text-sm leading-tight mb-1 group-hover:text-accent transition-colors" aria-hidden="true">
          {product.title}
        </h3>
        {lowestPrice !== null && (
